...
// A dialable North American number: area code and exchange both start 2-9, and
// neither N11 service codes (411, 911) nor any X11 area code is assignable.
const NANP_RX = /^([2-9]\d{2})([2-9]\d{2})(\d{4})$/;
const isNanp = (d) => NANP_RX.test(d) && !(d[1] === '1' && d[2] === '1');

// Recognises a number embedded in a messy cell: optional +1 or 1, optional
// parens, and any mix of spaces, dots or dashes between the groups.
const LOOSE_RX = /(?:\+?1[\s.\-]*)?\(?([2-9]\d{2})\)?[\s.\-]*([2-9]\d{2})[\s.\-]*(\d{4})(?!\d)/;

/**
 * Accept a phone number in any shape an export might carry and return its ten
 * digits, or '' when the value cannot be a dialable US number.
 *
 * Handles +1 / 1 / 00 / 011 prefixes, parens, spaces, dots and dashes in any
 * combination, extensions, Excel's numeric mangling ("7034399049.0" and
 * "7.034399049e+09"), and cells holding more than one number.
 */
function normPhone(v) {
  let s = raw(v);
  if (!s) return '';

  // Excel rewrites long digit strings as floats or scientific notation.
  if (/^\d?\.?\d+e[+-]?\d+$/i.test(s)) {
    const n = Number(s);
  ...
  // Strip an extension first, so its digits cannot be read as part of the number.
  const bare = s.replace(/[\s,;]*(?:x|ext|extn|extension|#)[\s.:]*\d+\s*$/i, '');

  let d = bare.replace(/\D/g, '');
  if (d.length > 10) d = d.replace(/^(?:011|00)/, '');       // international access code
  if (d.length === 11 && d[0] === '1') d = d.slice(1);        // US country code
  if (isNanp(d)) return d;

  // Fall back to the first plausible number in the raw text, which rescues
  // cells like "cell 703-439-9049 / home 571-309-6400".
  const m = LOOSE_RX.exec(bare);
  if (m) {
    const hit = m[1] + m[2] + m[3];
    if (isNanp(hit)) return hit;
  }
  return '';
}

/** The one output shape: +1703 439 9049 */
